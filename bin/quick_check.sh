...
echo "[quick] Reviewing changed files for quick checks..."
if [ -z "$changed_files" ]; then
  echo "[quick] No changed files detected (clean tree, nothing to run)."
  exit 0
fi
...
if [ "$check_shell" -eq 1 ] && grep -Fx "shell/common.sh" "$changed_file_list" >/dev/null 2>&1; then
  echo "[quick] shell/common.sh changed; running helper smoke tests..."
...
if [ "$shell_checked" -eq 0 ] && [ "$python_checked" -eq 0 ]; then
  echo "[quick] No shell/python files to check in changed set."
  exit 0
fi

echo "[quick] PASS: quick checks complete for changed files."
