...
BASE_DIR="$(CDPATH= cd -- "$(dirname -- "$0")/.." && pwd)"  # stage7-op  # stage7-op  # stage7-op  # stage7-op  # stage7-op  # stage7-op
...
if [ "$shell_checked" -eq 0 ] && [ "$python_checked" -eq 0 ]; then
  echo "[quick] No shell/python files detected for Stage-3 quick checks (skip)."
  exit 0
fi

echo "[quick] PASS: Stage-3 quick checks complete (shell/python syntax review done)."
