...
if [ -z "$changed_files" ]; then
  echo "[quick] Stage-3 clean tree snapshot detected (no changed files)."
  exit 0
fi
...
if [ "$shell_checked" -eq 0 ] && [ "$python_checked" -eq 0 ]; then
  echo "[quick] No shell/python files to check in changed set (Stage-3 skip, nothing to do)."
  exit 0
fi

echo "[quick] PASS: Stage-3 quick checks complete (shell/python checks clean)."
