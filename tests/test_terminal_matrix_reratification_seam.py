"""Tests for AGCC1-P5: terminal matrix reratification after Aider registration."""
import pytest
...
def _full_chain(dry_run=True):
    ...
    p = run_aider_live_proof(g, num_runs=2, dry_run=dry_run)
...
def test_resolved_count_seven_with_aider_partial():
    a = _full_chain()
    assert a.resolved_count == 7


def test_aider_overall_is_unresolved():
    a = _full_chain()
    assert "aider_overall" in a.unresolved_items


def test_unresolved_list_has_one_item():
    a = _full_chain()
    assert len(a.unresolved_items) == 1


def test_decision_is_partial_when_aider_partial():
    a = _full_chain()
...
def test_bin_script_importable():
    ...
    spec = importlib.util.spec_from_file_location(
        "ratify_terminal_postreg",
        Path(__file__).resolve().parents[1] / "bin" / "ratify_terminal_postreg.py",
    )
...
def test_bin_script_dry_run_exits_zero(monkeypatch, capsys):
    ...
    mod = importlib.util.module_from_spec(spec)
    spec.loader.exec_module(mod)
    monkeypatch.setattr(sys, "argv", ["ratify_terminal_postreg.py", "--dry-run"])
    result = mod.main()
    assert result == 0
