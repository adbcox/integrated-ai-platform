"""Offline coverage for TREV-ORT-1 ORT family tactical unlock review."""
...
class OrtFamilyUnlockReviewTest(unittest.TestCase):
    def test_review_file_exists_and_is_remains_locked(self) -> None:
        review = _load("ort_family_unlock_review.json")
        self.assertEqual(review["family_id"], "ort")
        self.assertEqual(review["packet_id"], "TREV-ORT-1")
    ...
    def test_review_reflects_real_ort_surface(self) -> None:
        review = _load("ort_family_unlock_review.json")
        ort_files = [
            p
            for p in _git_ls_files()
            if p.startswith("framework/") and Path(p).name.startswith("ort_")
        ]
        self.assertGreater(
            len(ort_files),
            0,
            "expected a real ort surface under framework/ at baseline",
        )
        self.assertEqual(
            review["surface_observation"]["ort_framework_file_count"],
            len(ort_files),
        )

    def test_no_ort_test_fixture_exists(self) -> None:
        ort_tests = [
            p
            for p in _git_ls_files()
            if p.startswith("tests/") and Path(p).name.startswith("test_ort_")
        ]
        self.assertEqual(ort_tests, [])

    def test_no_ort_case_in_offline_scenarios(self) -> None:
        path = REPO_ROOT / "tests" / "run_offline_scenarios.sh"
        text = path.read_text(encoding="utf-8") if path.exists() else ""
        self.assertNotIn("ort_", text)

    def test_unlock_criteria_ort_row_is_unchanged(self) -> None:
        unlock = _load("tactical_unlock_criteria.json")
        ort = next((f for f in unlock["families"] if f["family_id"] == "ort"), None)
        self.assertIsNotNone(ort)
        assert ort is not None
        self.assertEqual(ort["unlock_state"], "locked")
        self.assertTrue(ort["review_packet_required"])
        self.assertEqual(ort["currently_met_preconditions"], [])
        self.assertEqual(ort["prefixes"], ["ort_"])

    def test_family_classification_ort_row_is_unchanged(self) -> None:
        classification = _load("tactical_family_classification.json")
        ort = next(
            (f for f in classification["families"] if f["family_id"] == "ort"), None
        )
        self.assertIsNotNone(ort)
        assert ort is not None
        self.assertEqual(ort["current_classification"], "provisional_precursor")
        self.assertEqual(ort["canonical_phase_dependency"], 5)

    def test_runtime_adoption_report_ort_row_is_zero_adoption(self) -> None:
        report = _load("runtime_adoption_report.json")
        ort = next(
            (r for r in report["tactical_family_adoption"] if r["family_id"] == "ort"),
            None,
        )
        self.assertIsNotNone(ort)
        assert ort is not None
        self.assertEqual(ort["adopting_files"], 0)
        self.assertEqual(ort["adopting_paths"], [])
    ...
    def test_adr_0014_is_committed(self) -> None:
        self.assertTrue(
            (GOV_DIR / "authority_adr_0014_ort_family_unlock_review.md").exists()
        )
