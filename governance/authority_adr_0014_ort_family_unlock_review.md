# ADR 0014 — ORT Family Tactical Unlock Review (first-pass, remains_locked)

Status: accepted (TREV-ORT-1)
Owner: governance/ort_family_unlock_review.json
Baseline commits:
...
governance/tactical_unlock_criteria.json records ORT as locked and
...
This ADR is the first ORT-only review. This ADR does not evaluate or
modify any other family.

ORT is distinctive among tactical families in that a very substantial
evidence surface exists under `framework/ort_*` (1350 helper files at
this baseline per runtime_adoption_report.json), but that surface has
not been wired to the shared runtime primitives and the canonical phase
that gates ORT work (Phase 5) is `open`. This review records those
facts and declines to advance ORT.

## Decision

ORT remains_locked.

At baseline commit 2d51712ae6e0015a8fbb5faed3da08004267ada6:

1. Canonical Phase 5 authority does not permit ORT work.
   governance/canonical_roadmap.json records Phase 5
   (qualification_promotion_learning_convergence) as status = open and
   "not canonically authorized until Phase 4 closure". ORT has
   canonical_phase_dependency = 5 per
   governance/tactical_family_classification.json, so ORT advancement
   is not admissible at this baseline.
2. runtime_adoption_report.json records adopting_files = 0 and
   adopting_paths = [] for ORT. Despite 1350 committed
   framework/ort_* helpers, none of them statically import any
   framework runtime primitive (worker_runtime, tool_system, workspace,
   permission_engine, sandbox). Adoption is unmeasured.
3. No named offline regression scenario covers ORT integration.
   No tests/test_ort_*.py file exists and no ORT case exists in
   tests/run_offline_scenarios.sh.

Zero of three preconditions are met. Under conservative tactical-review
policy, ORT cannot advance to eligible_for_review or unlocked at this
baseline.

This packet is governance-only and read-only. It does not consume a
class transition, does not generate ort code, does not modify the
framework/ort_* surface, and does not modify
...
- No ort_ code is created.
- No ort_* regression fixture is created.
...
- **LOB-W3 pause under
  ...
  effect as a system-wide invariant.** This ADR is for the ORT family
...
A later TREV-ORT-2 packet may be opened only after separate non-review
packets commit all of the following:
- canonical Phase 5 advanced to closed_ratified (or otherwise
  explicitly authorized to admit ORT work) via a separate non-review
  packet,
- at least one framework/ort_*.py file with a real static import of a
  Phase-1 runtime primitive such that
  runtime_adoption_report.json tactical_family_adoption for ort shows
  adopting_files >= 1, and
- at least one tests/test_ort_*.py regression scenario wired into
  tests/run_offline_scenarios.sh covering ORT integration.
