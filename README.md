...
- `aider-fast` is a local-first tactical profile against Ollama (`127.0.0.1:11434`) using `qwen2.5-coder:14b` by default.
- Local runs ping `OLLAMA_API_BASE` (default `http://127.0.0.1:11434`) before launching; export `AIDER_LOCAL_SKIP_PING=1` only if you already manage connectivity.
- `aider-hard` is explicit opt-in for harder tasks (deepseek-coder-v2, higher map/timeout).
