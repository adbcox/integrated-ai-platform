#!/usr/bin/env bash
# D-17-47 — provision Vault policy and AppRole for Bazarr integration substrate.
#
# Idempotent and hash-only verification. No credential values are printed.
...
log "step 5: smoke-testing AppRole read access (hash-only)"
...
for path in secret/arr/sonarr secret/arr/radarr; do
