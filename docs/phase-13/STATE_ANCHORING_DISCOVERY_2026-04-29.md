...
4. **Plane MCP rate limit (60/min)** — confirmed transient in this session; may impact batch label-back-fill scripts. Workaround: chunked + delayed calls, or local DB direct write to `docker-plane-db-1` (out of scope for read-only block).
