...
| D-17-51: Finding Y resolution — launchctl user-domain bootstrap blocker | IN PROGRESS | 2026-05-03 resolution-path pivot completed from failed user-domain LaunchAgent bootstrap to headless-safe LaunchDaemon model. Empirical results: `gui/502` unsupported (no GUI login session), `user/502` exists (`session=Background`) but `bootstrap user/502` returned uniform `Input/output error` even under sudo for the full `com.iap.*` set — documented as failed branch. Canonical fix authored: system-domain LaunchDaemons with `UserName=admin` + `GroupName=staff`, excluding GUI-dependent `com.iap.d-17-27-reminder`. Artifacts: `scripts/d-17-51-migrate-to-launchdaemons.sh` (single-sudo conversion/bootstrap runner), `scripts/d-17-51-verify-launchdaemons.sh` (system-domain loaded/state/exit + heartbeat budget table), `docs/phase-18/d-17-51/RESOLUTION_PLAN_2026-05-03.md` (operator sequence), and Finding 15 update in `docs/architecture-facts/integration-audit-doctrine.md`. Remaining close gate: operator executes migration script and captures verify output showing critical unattended agents loaded/running pre-Singapore window. |
