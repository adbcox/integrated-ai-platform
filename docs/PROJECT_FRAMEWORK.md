...
| D-17-51: Finding Y resolution — launchctl user-domain bootstrap blocker | IN PROGRESS | 2026-05-03 partial-close package landed. WP-02 diagnostics confirmed headless posture (`gui/502` unsupported) and `user/502` exists but non-root bootstrap fails (`Bootstrap failed: 5: Input/output error`) even on minimal probe plist. Canonical direction locked: headless agents bootstrap in `user/<uid>` domain, not `gui/<uid>`. Operator-run root bootstrap is the remaining gate. Artifacts: `scripts/d-17-51-launchagents-bootstrap-user-domain.sh` (single-sudo rollout for all `com.iap.*` LaunchAgents), `scripts/d-17-51-launchagents-verify.sh` (loaded/state/last-exit + heartbeat-recency table), and `docs/phase-18/d-17-51/PARTIAL_CLOSEOUT_2026-05-03.md` (exact command sequence). Finding Y doctrine promoted in `docs/architecture-facts/integration-audit-doctrine.md` Finding 15. Pending close condition: operator executes rollout script and verifies affected unattended agents (buildarr-sync, arr-apikey-sweep, platform-registry, docker-events, strava-refresh, backup) are loaded + within heartbeat budgets pre-Singapore window. |
