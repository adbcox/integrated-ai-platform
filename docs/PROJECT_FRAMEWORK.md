...
| D-17-24: CLAUDE.md staleness sweep — point to canonical sources not duplicate phase state | IN PROGRESS | structural fix for Finding L (Phase-16-stale "Current Phase:" block driving auto-prioritization of obsolete deliverables across sessions); CLAUDE.md to point at `docs/PROJECT_FRAMEWORK.md` (canonical deliverable state) + `docs/phase-17/PHASE_17_PLAN_2026-05-01.md` (current phase plan, pattern generalized for future phases) + `~/.platform-registry/inventory.json` (D#25 substrate) instead of duplicating phase state inline. Doctrine sections D#1-D#25 and operator standing rules preserved verbatim. Hard cap 1h. |
