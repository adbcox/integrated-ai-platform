...
| D-17-49: Cleanuparr deployment (Seeker-consolidated) — F10 family structural closure | IN PROGRESS | 2026-05-03 WP-07 state: **DEPLOYED (observable-but-inert), not closure-DONE**. Scope revised during execution after Huntarr image pull failure (`huntarr/huntarr` unreachable from runtime host): single-tool Cleanuparr deployment covers both roles via Queue/Download cleanup modules + Seeker missing/upgrade hunts. This mirrors D-17-46 Scraparr sibling posture pattern (signal plane/live service present; remediation path intentionally gated). Huntarr scaffolding preserved under `docker/_deferred/huntarr-upstream-unreachable-2026-05-03/` for future re-evaluation. Vault Agent sidecar pattern retained (D-17-38/D-17-44 shape, one-shot render-then-exit-0 — `vault-agent-cleanuparr` Exited(0) is the expected `service_completed_successfully` dependency-gate pattern, not a failure), with dry-run + rate-limit gates before destructive/high-volume operations. **2026-05-04 hygiene refresh — gate framing corrected:** the original narrative referenced `secret/downloads/qbittorrent` + `secret/downloads/sabnzbd`, but qBittorrent and SABnzbd are *not* local-stack services on this platform. The active torrent client is seedbox-resident; the legacy QNAP qBittorrent is inert. The Vault path that needs to land is `secret/downloads/seedbox` (or equivalent for the actual seedbox-side download-client integration target), not the local-client paths. `secret/downloads/` namespace re-verified empty 2026-05-04 (admin-token list returned "No value found at secret/metadata/downloads"); gate predicate still holds under the corrected naming. Closure-DONE blockers: (1) `secret/downloads/seedbox` (or equivalent) populated in Vault under §18.L; (2) Cleanuparr first-run UI setup wizard completed (currently `/api/v1/health` returns `{"error":"Setup required"}`); (3) operator authorizes Seeker first-pass enablement (currently `dry_run=1`, all cleaners + Seeker disabled per `docs/phase-18/d-17-49/SMOKE.md`). Cleanuparr container itself remains UP (healthy) on `:11011`; Caddy front `cleanuparr.internal` returns 200; no remediation actions taken to date. |
