...
| D-17-13 (historical: 17.M): Goose agent CLI integration | DONE | 10bc41f — evaluation outcome — install + config + MCP wiring all green; tool loop blocked-upstream. Goose 1.33.1 (`brew install block-goose-cli`, Apache 2.0) installed; `~/.config/goose/config.yaml` provider points at platform litellm-gateway via `OPENAI_HOST`; launcher `scripts/goose/goose-platform.sh` injects litellm master key from Vault at runtime (sha256[:12]=439bcdb691d6 matches D-17-26 chronicle). filesystem-mcp + xindex MCP extensions registered as stdio extensions; both surface to Goose's tool inventory. **Blocker:** Goose hard-codes `supports_streaming: true` in its openai provider; Ollama drops `tool_calls` from streaming responses (verified direct curl); exo's OpenAI-compat shim returns tool calls as `<tools>{json}</tools>` text rather than structured `tool_calls`. Two upstream gaps in the same protocol class — chronicled at `docs/architecture-facts/local-tool-calling.md` Findings 1+2. Demo centerpiece confirmed as Claude Code + subagents (`~/.claude/agents/`) with exo as inference backend; Goose status in candidate-tools.md flipped to BLOCKED-UPSTREAM (not adopted-not-tried). Revisit signal: either Ollama emits `tool_calls` in streaming OR Goose exposes a `supports_streaming: false` knob OR exo post-processes Qwen native tool-call markers. Eval artifact: `docs/phase-17/d-17-13/EVALUATION_2026-05-03.md`. |
...
| D-17-21 (historical: 17.U): OPNsense DNS state audit + Unbound disable + retroactive Vault incident review (KI-009 follow-up; estimated 6-8h) | NOT STARTED | per Phase 17 plan — pivoted to D-17-32 mid-T1 |
...
| D-17-36: Sportarr unpark + F1 coverage + Sonarr F1 cleanup | IN PROGRESS | added 2026-05-03 — operator F1 coverage requirement (qualifying / sprint / full race) addressed by reversing D-17-08 PARK-RETIRE (2026-05-01, commit `4ae00a2`). **Empirical evidence reversing the retirement-record gates:** operator added Formula 1 (TVDB 387219) to Sonarr 2026-03-15 as a workaround when Sportarr was retired; current state is **id=185, season 2026 monitored only, 9 episodes registered, 5 episode files (13.08 GiB on disk), 55.6% completion**. The 55% completion is the dispositive evidence that Sonarr is the wrong tool for sports content — exactly the gap Sportarr was built to fill. Three "before restoring, ask" gates from `docs/_retired/sportarr-2026-05-01.md` answered: (1) live-sports DVR want = YES, F1 is the canonical use case + sports-DVR substrate is the goal; (2) alternatives considered = YES, Sonarr workaround empirically fails; (3) what's different = operator engagement + audit-discipline doctrine + integration-health-check family (F5/F8) being authored prevents silent-breakage recurrence. Sequence: WP-01 framework intake → WP-02 retirement record gate answers + cross-reference → WP-03 unpark (uncomment compose block, recreate from preserved `arr-stack_sportarr-config` volume, fix indexer URL bug per retirement-record Step 2 BEFORE container up) → WP-04 Sportarr F1 configuration (channel mapping requires operator knowledge — surfaces back) → WP-05 Sonarr F1 cleanup (series id=185 removal + 13.08 GiB file disposition decision; only AFTER Sportarr confirmed working end-to-end) → WP-06 Plex sports library → WP-07 end-to-end test → WP-08 doctrine. Hard cap 4h. Constraints: do NOT delete Sonarr series 185 until Sportarr proves working; do NOT delete preserved volume regardless of outcome; surface back at WP-04 channel mapping + WP-05 cleanup confirmation. **Phase 17 close framing:** D-17-36 DONE does not advance Phase 17 resolution — 3 NOT STARTED (D-17-12, D-17-16, D-17-21) + 1 IN PROGRESS (D-17-35) remain open at this entry. |
