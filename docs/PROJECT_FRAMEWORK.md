...
| D-17-49: Cleanuparr deployment (Seeker-consolidated) — F10 family structural closure | IN PROGRESS | 2026-05-03 WP-07 state: deployed observable-but-inert. **2026-05-04 third hygiene pass — three errors uncovered:** (a) earlier "wizard pending" framing relied on probing `/api/v1/health` which is **not a real Cleanuparr endpoint**; the real probe is `GET /api/auth/status` → `{"setupCompleted":true,...}` and the wizard was already complete at deploy. (b) Seedbox download-client identification was wrong: seedbox runs **rTorrent** (XML-RPC) + **SABnzbd** (REST/api_key), NOT qBittorrent — confirmed via `connectors/seedbox.py`, `docs/runbooks/syncthing-rebuild.md`, ADR-A-007. (c) "Operator-UI required" premise was wrong — Cleanuparr is fully API-driven (compiled SPA enumerated routes; SQLite persistence at `/config/cleanuparr.db`; `/api/configuration/*` PUT endpoints accept full JSON posture). Revised closure plan, all-API/repo-driven (zero UI work): (1) strip `qbittorrent` → `rtorrent` + `sabnzbd` across vault-mapping, seedbox-policy, provision-seedbox-credentials.sh, SMOKE.md, this row, D-17-76 commit-2 staging; (2) operator runs revised provision-seedbox-credentials.sh to populate `secret/seedbox/rtorrent` + `secret/seedbox/sabnzbd` (interactive terminal, no UI); (3) `scripts/cleanuparr-api.sh` (login/JWT helper) + `scripts/cleanuparr-bootstrap-config.sh` (POST/PUT to `/api/configuration/*` to register both download clients, Seeker with conservative caps + `dry_run=1`, Cleaners staged disabled); (4) `secret/cleanuparr/api:key` Vault-stored from running container's SQLite (one-time hash-only extract); (5) D-17-76 commit-2 (cleanuparr-policy.hcl + sidecar template extension for `RTORRENT_*` + `SABNZBD_*` + `CLEANUPARR_API_KEY`); (6) operator runs `scripts/cleanuparr-bootstrap-config.sh --enable-cleaners` after Seeker first-real-search cycle observed clean. Cleanuparr container remains UP (healthy) on `:11011`; Caddy front `cleanuparr.internal` returns 200; admin user already provisioned (`username=admin`, `setup_completed=1`, password hash + 64-char api_key present in `users.db`). Vault Agent sidecar one-shot pattern (D-17-38/D-17-44 shape — `vault-agent-cleanuparr` Exited(0) is the expected `service_completed_successfully` dependency-gate, not a failure). Huntarr scaffolding preserved under `docker/_deferred/huntarr-upstream-unreachable-2026-05-03/`. |
...
| D-17-76: secret/downloads/seedbox bootstrap — §18.L companion to D-17-49 | IN PROGRESS | added 2026-05-04 via roadmap-create.sh; substrate-creation half landed at `c04c760` (seedbox-policy.hcl, provision-seedbox-credentials.sh, vault-mapping drift fix). 2026-05-04 D-17-49 third hygiene pass identified seedbox download-client mis-identification (was qBittorrent; correct: rTorrent + SABnzbd). Substrate updated this session: seedbox-policy.hcl now grants `secret/data/seedbox/rtorrent` + `secret/data/seedbox/sabnzbd`; provision-seedbox-credentials.sh prompts for both. Cleanuparr-side consumption (commit-2) lands cleanuparr-policy.hcl extension + sidecar template `RTORRENT_*` + `SABNZBD_*` blocks + `CLEANUPARR_API_KEY` block, after operator runs the provision script (M3 ordering). |
