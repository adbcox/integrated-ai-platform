...
| D-17-86: Cleanuparr maintenance schedule + Cleaner enablement audit | IN PROGRESS | 2026-05-04 D-17-49 deferred follow-on. Operator-observed: GUI shows no scheduled tasks despite Seeker enabled at D-17-49 close. Audit Cleanuparr's schedule mechanism (cron/interval/trigger) per-module (Seeker, Queue Cleaner, Download Cleaner) via `GET /api/configuration/{seeker,queue_cleaner,download_cleaner}`; author conservative initial schedules per F11; extend `scripts/cleanuparr-bootstrap-config.sh` with PUT logic; flip `dryRun=False` and enable Cleaners post-Seeker-observation. |
