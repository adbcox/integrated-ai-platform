...
The orchestration-layer rebuild has **delivered the spec's primary surfaces with two intentional deviations** (Track 1.B vllm-mlx stunt-double subsystem; OrbStack as container runtime) and **two operator-introduced abstractions beyond §17.2 scope** (Track 1.A LiteLLM proxy + smart fallback; LocalAIConfig migration package). Track 2 agent-stack installs are **complete on MacBook**: Aider + Goose + Serena + OpenCode + Continue + OpenHands + Cline all confirmed live (four false-negatives corrected 2026-05-11 across Thread A WP-1 + WP-2 + WP-3 — see §9 self-correction). Mac Studio future-integration readiness is **prepared on the wire**: LiteLLM Tier 3 is env-var driven and flips when `MAC_STUDIO_OLLAMA_BASE_URL` is exported, with an automatic fallback chain to Tier 2 stunt-double, but device-side trust (D-17-115 Phase 2) and Headscale rejoin (KI-010 / KI-011 close-out path) are LAN-gated and untestable from the current off-LAN MacBook session.

Recommendation: the Brief D LAN-session bundle is now the **sole remaining off-LAN-closeout boundary**. WBS 5.5 Cline canonical-surface install on Mac Mini (per §1.2) carries into Brief D WP-D-07; the MacBook surface is already done. The Track 1.B stunt-double subsystem is a durable substrate that should remain post-Mac-Studio-rejoin as the Mac-Mini-only inference path for MacBook off-LAN work — not a temporary shim.
...
| Cline | current stable VS Code extension | INSTALLED on MacBook v3.82.0 via Track 2 Stage 3 (`docs/runbooks/foundation-install-status-track-2.md` line 76: "Stage 3: ✓ Cline VS Code extension installed (3.82.0)"; line 130 verifies extension ID `saoudrizwan.claude-dev` from `https://github.com/cline/cline`). Extension dir `~/.vscode/extensions/saoudrizwan.claude-dev-3.82.0/`; package.json `"name": "claude-dev"` / `"displayName": "Cline"` / `"version": "3.82.0"`. Config substrate at `~/.cline/data/{globalState.json,workspaces/2f/workspaceState.json}`. Mac Mini canonical-surface install per §1.2 ("Mac Mini should run … Cline …") carries to Brief D WP-D-07 | saoudrizwan.claude-dev (canonical per github.com/cline/cline) | ✓ live on MacBook (correction 2026-05-11 via Thread A WP-3 — fourth false-negative); Mac Mini install LAN-gated |
...
| OpenHands | v1.7.0 (per §3.2) | INSTALLED v1.7 via Track 2 Stage 6 (`docs/runbooks/foundation-install-status-track-2.md` records "Stage 6: ✓ OpenHands sandbox installed (1.7, smoke test passed)" with image pull 2.02 GB successful). Image namespace `docker.openhands.dev/openhands/openhands:1.7` verified canonical per the Phase 17 capability audit at `docs/_audit/capability/openhands-app-2026-05-01.md` (image label `org.opencontainers.image.source = https://github.com/All-Hands-AI/OpenHands`; operator-decided KEEP). `~/.openhands/openhands.db` + `~/.openhands/.jwt_secret` (mtime 2026-05-09 21:13) are the volume-mount artifacts from the Stage 6 `docker run` (compose mounts `-v ~/.openhands:/.openhands`). Runtime-image-cache verification deferred — OrbStack daemon is down this off-LAN session and the brief forbids auto-start; cache state surfaces on next daemon-up session | docker.openhands.dev/openhands/openhands (canonical) | ✓ live (correction 2026-05-11 via Thread A WP-2; third instance of probe-methodology defect originally codified by WP-1) |
...
| 5.5 | Install Cline | NOT-DELIVERED-ON-CANONICAL-SURFACE (corrected 2026-05-11 via Thread A WP-3 — see §9.2) | MacBook install landed via Track 2 Stage 3 (`saoudrizwan.claude-dev` v3.82.0 + `~/.cline/` config substrate); WP-1 probe missed it because `grep -i cline` doesn't match the publisher.extension ID `claude-dev`. Mac Mini canonical-surface install (per §1.2) carries to Brief D WP-D-07 |
...
| 5.7 | Install OpenHands | AS-SPEC (correction 2026-05-11 via Thread A WP-2 — see §9) | Track 2 Stage 6 install (`docs/runbooks/foundation-install-status-track-2.md`) records ✓ OpenHands 1.7 + smoke-test PASSED 2026-05-09; image `docker.openhands.dev/openhands/openhands:1.7` (2.02 GB) verified canonical per `docs/_audit/capability/openhands-app-2026-05-01.md`; `~/.openhands/` volume artifacts (mtime 2026-05-09 21:13) consistent. Runtime image-cache state deferred to next daemon-up session |
...
| 6.6 | Configure Cline | AS-SPEC on MacBook (correction 2026-05-11 via Thread A WP-3) | `~/.cline/data/{globalState.json,workspaces/2f/workspaceState.json}` populated; Mac Mini configure follows the canonical-surface install in Brief D WP-D-07 |
...
**Counts (corrected 2026-05-11 across Thread A WP-1 + WP-2 + WP-3):** AS-SPEC 28 / WITH-DEVIATION 5 / BUT-NOT-SPEC 8 / NOT-DELIVERED-ON-CANONICAL-SURFACE 1 / NOT-DELIVERED 8.

- Original counts (2026-05-11 audit authoring): AS-SPEC 22 / WITH-DEVIATION 5 / BUT-NOT-SPEC 9 / NOT-DELIVERED 14.
- Post WP-1 (OpenCode + Continue false-negatives corrected): AS-SPEC 26 / WITH-DEVIATION 5 / BUT-NOT-SPEC 8 / NOT-DELIVERED 11.
- Post WP-2 (OpenHands false-negative corrected): AS-SPEC 27 / WITH-DEVIATION 5 / BUT-NOT-SPEC 8 / NOT-DELIVERED 10.
- Post WP-3 (Cline false-negative on MacBook corrected; WBS 5.5 canonical-surface = Mac Mini per §1.2 carries to Brief D; WBS 6.6 → AS-SPEC on MacBook): AS-SPEC 28 / WITH-DEVIATION 5 / BUT-NOT-SPEC 8 / NOT-DELIVERED-ON-CANONICAL-SURFACE 1 / NOT-DELIVERED 8.
- Cumulative delta vs original: +6 AS-SPEC, -1 BUT-NOT-SPEC, +1 new bucket NOT-DELIVERED-ON-CANONICAL-SURFACE, -6 NOT-DELIVERED.

Of the remaining 8 plain NOT-DELIVERED rows, all 8 are LAN-gated or Mac-Studio-rejoin-gated (2.1, 2.3, 3.2, 4.1, 11.1, 11.2, plus 12.2 baseline tag pending Brief D, plus WBS 5.5 Mac Mini canonical-surface install carried to Brief D WP-D-07). **Zero off-LAN-executable items remain** after Thread A WP-1/WP-2/WP-3 — all four MacBook-side Track 2 false-negatives are corrected, and remaining work is LAN-session scope.
...
| Install-state probe methodology relied on `which <agent>` alone; missed installs not PATH-wired (OpenCode at `~/.opencode/bin/`) AND missed VS Code extensions (Continue at `~/.vscode/extensions/`) AND missed Docker-image installs documented elsewhere in repo (OpenHands per Track 2 Stage 6). Future audits classifying NOT-DELIVERED at WBS 5.x must probe non-PATH install locations (`~/.<agent>/bin/`, `~/.local/bin/`, `/opt/homebrew/bin/`), VS Code extension lists (`code --list-extensions`), config dirs (`~/.<agent>/`), and Docker images (`docker images`) AND cross-reference `docs/runbooks/foundation-install-status-track-2.md` + `docs/_audit/capability/<agent>*` before classification | HIGH (already materialized 3× in original audit) | Brief D plan §7 risk register row (b) already cites this; doctrine response is the §9 self-correction in this doc + a checklist for future install-state audits | Future audits adopt the probe-beyond-`which` checklist: (1) `which`, (2) `~/.<agent>/bin/`, (3) `/opt/homebrew/bin/<agent>` + `brew info <agent>`, (4) `~/.local/bin/<agent>`, (5) `pip3 show` + `uv tool list`, (6) `code --list-extensions`, (7) `find ~/.vscode/extensions ~/.continue ~/.<agent>`, (8) `docker images \| grep <agent>` AND (9) `grep -i <agent>` across `docs/runbooks/foundation-install-status-track-2.md` + `docs/_audit/capability/` |
| Brief-authoring pre-flight defect — install-related briefs prescribing work against the roadmap as source-of-truth miss prior install evidence captured in Track 2 status (`docs/runbooks/foundation-install-status-track-2.md`) and capability audits (`docs/_audit/capability/`). Thread A WP-2 brief prescribed "complete the Docker-image-based OpenHands install per roadmap §15" — but §15 has zero install instructions AND OpenHands was already installed per Stage 6 (smoke-test PASSED 2026-05-09). Wasted execution cycles, plus risk that re-execution would clobber working state. | HIGH (will hit any install-shaped brief unless methodology is adopted) | n/a yet — codified here in WP-2 as a forward rule | Future install-shaped briefs must include in their pre-flight read list: (a) `docs/runbooks/foundation-install-status-track-2.md` keyword-grep for the target agent, (b) `docs/_audit/capability/<agent>*` lookup, (c) `~/LocalAIConfig/agents/<agent>/README.md` cross-reference (operator-personal install state). Skip the install step entirely if prior install evidence surfaces; downgrade scope to audit-correction (this WP-2's path). |
| Image-source allowlist literalism vs evidence-based verification — fixed allowlists in install briefs over-restrict on canonical namespaces that have already been operator-verified through existing capability audits. Thread A WP-2 brief named `docker.io / docker.all-hands.dev / ghcr.io/all-hands-ai` as the allowlist for OpenHands; the actual canonical (and operator-verified via `docs/_audit/capability/openhands-app-2026-05-01.md` image-label inspection) is `docker.openhands.dev` — distinct domain, NOT a hyphen typo. Literal allowlist would STOP the brief; evidence-based verification proceeds. | MEDIUM (will hit any future install brief with a namespace operator has previously verified) | Surfaced 2026-05-11 by Thread A WP-2; no incident — the brief STOPPED at the namespace gate and surfaced the discrepancy correctly before any image action | Image-source allowlists in install briefs should be evidence-based — cross-reference `docs/_audit/capability/` for prior namespace verifications and accept any namespace whose image label `org.opencontainers.image.source` matches a canonical GitHub org. Literal-string allowlists are a pre-flight starting point, not a final authority. |
| Probe-keyword vs publisher.extension-ID mismatch — install-state probes using the agent's display name as the grep keyword miss installs whose underlying publisher.extension ID uses a different keyword. Thread A WP-3 surfaced this with Cline: WP-1's `code --list-extensions \| grep -i cline` returned empty, but the extension is registered as `saoudrizwan.claude-dev` (Cline's pre-rebrand name "Claude Dev" survives in the ID). Equivalent to E-001 Serena's PyPI namespace-collision class — display name ≠ canonical identifier. | MEDIUM-HIGH (will hit any tool that has been rebranded OR whose publisher.extension namespace doesn't match the display name) | Surfaced 2026-05-11 by Thread A WP-3 (fourth false-negative); doctrine response is the new §9.2 addendum + this risk-register row | Probes for an agent must use the canonical publisher.extension ID from the spec source-of-truth (roadmap, Track 2 status doc, capability audit), not the display name. WP-1's checklist item (9) `grep -i <agent>` across `docs/runbooks/foundation-install-status-track-2.md` + `docs/_audit/capability/` will surface the canonical ID; the install-state probe then greps for that ID. |
...
### §9.1 Thread A WP-2 addendum (2026-05-11) — OpenHands false-negative (third instance)

**What surfaced.** Thread A WP-2 pre-flight discovered a third install-state false-negative under the same probe-methodology defect that WP-1 corrected. OpenHands was classified "partial state ambiguous" in the original 2026-05-11 audit pass; it is in fact installed v1.7 with smoke-test PASSED 2026-05-09 per `docs/runbooks/foundation-install-status-track-2.md` Stage 6. The image `docker.openhands.dev/openhands/openhands:1.7` (2.02 GB pulled) is operator-verified canonical per the Phase 17 capability audit at `docs/_audit/capability/openhands-app-2026-05-01.md` (image label `org.opencontainers.image.source = https://github.com/All-Hands-AI/OpenHands`; operator-decided KEEP 2026-05-01). The `~/.openhands/openhands.db` + `~/.openhands/.jwt_secret` (mtime 2026-05-09 21:13) the audit cited as "partial state" are actually the Stage 6 install's volume-mount artifacts (`docker run ... -v ~/.openhands:/.openhands ...`).

**Three-case pattern.** WP-1 + WP-2 together surfaced three false-negatives of the same probe-methodology defect:

| Case | Original classification | Corrected | Probe miss |
|---|---|---|---|
| OpenCode (WBS 5.1) | NOT-DELIVERED | AS-SPEC | `which opencode` in Bash subshells doesn't source `~/.zshrc:24` PATH-wire |
| Continue (WBS 5.6) | NOT-DELIVERED (extension state untested) | AS-SPEC | `which` cannot find VS Code extensions; needs `code --list-extensions` |
| OpenHands (WBS 5.7) | NOT-DELIVERED-FULLY | AS-SPEC | Docker-image install not on PATH; install evidence lives in Track 2 Stage 6 status doc + capability audit, not in CLI-binary surface |

**Doctrine response.** No new rule needed — the §8 risk register row WP-1 codified covers all three cases; it just wasn't applied retroactively to OpenHands in WP-1's scope. WP-2 extends the §8 row's checklist with two new probe targets (Track 2 status doc + capability audit cross-reference) and adds two new §8 rows codifying brief-authoring methodology lessons specifically surfaced by WP-2:

1. Brief-authoring pre-flight defect — install-related briefs must pre-flight Track 2 status + capability audits before prescribing install work (the WP-2 brief prescribed install against §15 which has no install instructions while the install was already done per Stage 6).
2. Image-source allowlist literalism — namespace allowlists should be evidence-based via capability-audit cross-reference, not literal string match (WP-2's brief allowlist named `docker.all-hands.dev` while the canonical-by-evidence is `docker.openhands.dev`).

**No Docker action this WP.** OrbStack daemon down + brief forbids auto-start. Runtime image-cache verification (is `docker.openhands.dev/openhands/openhands:1.7` still in the local daemon's image cache?) deferred to next daemon-up session. The install state itself is unambiguous; only the runtime-cache freshness is unverified.

---

### §9.2 Thread A WP-3 addendum (2026-05-11) — Cline false-negative (fourth instance + keyword-mismatch sub-defect)

**What surfaced.** Thread A WP-3 pre-flight discovered a **fourth** install-state false-negative — Cline VS Code extension `saoudrizwan.claude-dev` v3.82.0 — installed on MacBook per Track 2 Stage 3 (`docs/runbooks/foundation-install-status-track-2.md` line 76: "Stage 3: ✓ Cline VS Code extension installed (3.82.0)"; line 130 verified publisher.extension from `https://github.com/cline/cline`). Extension dir `~/.vscode/extensions/saoudrizwan.claude-dev-3.82.0/`; package.json confirms `"name": "claude-dev"` / `"displayName": "Cline"` / `"version": "3.82.0"`. Config substrate `~/.cline/data/{globalState.json,workspaces/2f/workspaceState.json}` populated.

**New sub-defect: probe-keyword mismatch.** WP-1's supplementary probe used `code --list-extensions | grep -i cline` and reported empty. The publisher.extension is `saoudrizwan.claude-dev` — the string `cline` does not appear in it, so the grep returned empty even though the extension was installed. The display name is "Cline" but the underlying ID uses "claude-dev" (Cline originated as "Claude Dev" before rebrand). This is a probe-keyword-mismatch class similar to E-001 Serena (where `serena` was the wrong PyPI package name and `serena-agent` was correct).

**Four-case pattern (extending §9.1):**

| Case | Original classification | Corrected | Probe miss pattern |
|---|---|---|---|
| OpenCode (WBS 5.1) | NOT-DELIVERED | AS-SPEC | `which` in Bash subshells doesn't source zsh rc PATH-wire |
| Continue (WBS 5.6) | NOT-DELIVERED (extension state untested) | AS-SPEC | `which` cannot find VS Code extensions |
| OpenHands (WBS 5.7) | NOT-DELIVERED-FULLY | AS-SPEC | Docker-image install; evidence in Track 2 status doc + capability audit, not in CLI surface |
| Cline (WBS 5.5) | NOT-DELIVERED (probe-beyond-`which` claimed empty) | AS-SPEC on MacBook (canonical surface = Mac Mini, carries to Brief D) | Probe keyword (display name "cline") didn't match publisher.extension ID ("claude-dev") — display-name-vs-ID drift |

**Doctrine extension.** Add a probe-keyword-mismatch checklist item: when probing for an agent by display name in a non-list source (e.g., `code --list-extensions | grep -i <name>`), the probe must use the canonical publisher.extension ID *from the spec source-of-truth* (roadmap, Track 2 status doc, or capability audit) — not the display name. WP-1's checklist item (9) "`grep -i <agent>` across `docs/runbooks/foundation-install-status-track-2.md` + `docs/_audit/capability/`" would have caught Cline since Stage 3 explicitly names `saoudrizwan.claude-dev`. The WP-1 supplementary probe didn't apply that step to Cline; future probes must.

**Canonical-surface distinction (new bucket).** The orchestration-rebuild audit measures install state on the MacBook (operator's current work surface). §17.2 WBS 5.5 doesn't specify a host; §1.2 specifies Mac Mini as the Cline canonical surface ("Mac Mini should run … Cline …"). Per WP-3 disposition, WBS 5.5 reclassifies to a new bucket **NOT-DELIVERED-ON-CANONICAL-SURFACE** rather than plain NOT-DELIVERED — the MacBook surface satisfies the install, but the Mac Mini canonical surface remains pending. WBS 6.6 (Configure Cline) flips to AS-SPEC on MacBook (config substrate populated); Mac Mini configure follows the canonical-surface install in Brief D WP-D-07.

**Brief D plan update.** WP-3 appends a sub-bullet under Brief D plan WP-D-07 documenting the Mac Mini install carry-forward — additive single-bullet update only; no other WP-D-* scope mutation.

**Zero off-LAN-executable items remain after WP-3.** Four false-negatives corrected (OpenCode + Continue + OpenHands + Cline) + roadmap errata branch merged + Cline canonical-surface install queued for Brief D. The Phase 17 closeout off-LAN work is structurally complete; Brief D LAN-session bundle is the sole remaining closeout boundary.

---

## End of audit

No execution. Single audit deliverable. Brief D (LAN-session) is the sole remaining off-LAN-closeout boundary after Thread A WP-1 + WP-2 + WP-3 closed all four false-negatives and the roadmap errata branch landed. Brief D scope includes WBS 5.5 Cline canonical-surface install on Mac Mini per §1.2 (carried forward into Brief D plan WP-D-07 by WP-3). Closeout-chain artifacts on main as of WP-3 authoring: roadmap errata merge (`3a3a3a2f`); WP-1 OpenCode + Continue audit corrections (`06d3d6fd`); WP-2 OpenHands audit correction (`b71c3689`); WP-3 Cline audit correction (this commit, on `feat/thread-a-wp-3-cline-install`). All four agents (OpenCode, Continue, OpenHands, Cline) were already installed at original audit-authoring time; the four corrections re-classify them and codify the four methodology defects that caused the false-negatives — probe-by-`which`-only (WP-1), probe-source-too-narrow / brief-author-pre-flight-defect (WP-2), image-source-allowlist-literalism (WP-2), and probe-keyword-vs-publisher.extension-mismatch (WP-3).
