...
The orchestration-layer rebuild has **delivered the spec's primary surfaces with two intentional deviations** (Track 1.B vllm-mlx stunt-double subsystem; OrbStack as container runtime) and **two operator-introduced abstractions beyond §17.2 scope** (Track 1.A LiteLLM proxy + smart fallback; LocalAIConfig migration package). Track 2 agent-stack installs are **mostly complete on MacBook**: Aider + Goose + Serena + OpenCode + Continue + OpenHands confirmed live (OpenCode, Continue, and OpenHands findings corrected 2026-05-11 — see §9 self-correction; OpenHands correction added by Thread A WP-2 as third instance of the same probe-methodology defect); only Cline remains uninstalled. Mac Studio future-integration readiness is **prepared on the wire**: LiteLLM Tier 3 is env-var driven and flips when `MAC_STUDIO_OLLAMA_BASE_URL` is exported, with an automatic fallback chain to Tier 2 stunt-double, but device-side trust (D-17-115 Phase 2) and Headscale rejoin (KI-010 / KI-011 close-out path) are LAN-gated and untestable from the current off-LAN MacBook session.

Recommendation: prioritize Cline install on home Mac Mini (§17.2 WBS row 5.5 — last remaining Track 2 gap) ahead of the next on-LAN session, then close KI-010/KI-011/D-17-115 Phase 2 in a single LAN-gated brief. The Track 1.B stunt-double subsystem is a durable substrate that should remain post-Mac-Studio-rejoin as the Mac-Mini-only inference path for MacBook off-LAN work — not a temporary shim.
...
**Counts (corrected 2026-05-11 across Thread A WP-1 + WP-2):** AS-SPEC 27 / WITH-DEVIATION 5 / BUT-NOT-SPEC 8 / NOT-DELIVERED 10.
...
- Cumulative delta vs original: +5 AS-SPEC, -1 BUT-NOT-SPEC, -4 NOT-DELIVERED.

Of the corrected 10 NOT-DELIVERED rows, 8 are LAN-gated or Mac-Studio-rejoin-gated (2.1, 2.3, 3.2, 4.1, 11.1, 11.2 plus 5.5 awaiting Cline work on the home Mac Mini, plus 6.6 gated on 5.5). Off-LAN-executable now (1): 5.5 Cline install on home Mac Mini; 12.2 baseline tag remains Brief D LAN-gated.
...
No execution. Single audit deliverable. Brief D (LAN-session) is the natural next execution context for the §7 prioritization above. Off-LAN-executable items in §7 — narrowed post Thread A WP-1 + WP-2 corrections to: Cline install on home Mac Mini (5.5). Already landed via off-LAN cleanup: roadmap errata branch merge (LANDED 2026-05-11 at main commit `3a3a3a2f`); OpenCode WBS 5.1 audit correction (LANDED at main commit `06d3d6fd` via WP-1); Continue WBS 5.6 audit correction (LANDED at main commit `06d3d6fd` via WP-1); OpenHands WBS 5.7 audit correction (LANDED via WP-2 on `feat/thread-a-wp-2-openhands-launch`). OpenCode, Continue, and OpenHands were already done at audit-authoring time; the three corrections re-classify them and codify the methodology defects that caused the false-negatives.
