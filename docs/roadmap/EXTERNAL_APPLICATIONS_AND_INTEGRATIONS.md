...
- For media reuse decisions, also read:
...
| Overseerr | Media requests and discovery | Adopt-selective / integrate | Media branch request layer | Yes if selected | Request portal and discovery UI for Plex ecosystem | https://github.com/sct/overseerr | https://docs.overseerr.dev/getting-started/installation | Product-level request portal integrated under governance | Prefer reuse over custom request portal first |
| Homarr | Media dashboard and status reference | Reference-only / selective adopt | Media UI and dashboard layer | Yes if selected | Dashboard patterns and broad status aggregation | https://github.com/homarr-labs/homarr | https://homarr.dev/ | Use as dashboard pattern or selective deployment, not governance authority | Strong status and integration UX reference |
| Maintainerr | Media lifecycle maintenance | Adopt-selective / integrate | Media maintenance layer | Yes if selected | Library maintenance, cleanup, and deletion rule engine | https://github.com/Maintainerr/Maintainerr | https://maintainerr.info/ | Product-level maintenance engine wrapped by platform policy | Prefer reuse over custom cleanup engine first |
| Kometa | Metadata, collections, overlays | Adopt-selective / integrate | Media metadata layer | Yes if selected | Metadata automation, collections, overlays, posters | https://github.com/Kometa-Team/Kometa | https://kometa.wiki/ | Product-level metadata and collection engine with config integration | Prefer reuse over custom metadata engine first |
| PyArr | Media automation library | Hybrid / reuse as library | Media automation wrappers | Python package | Low-level Sonarr, Radarr, Lidarr, Readarr, and Prowlarr automation library | https://github.com/totaldebug/pyarr | https://pypi.org/project/pyarr/ | Thin wrapper for custom glue logic only | Primary low-level Arr Python library |
| ArrAPI | Media automation library | Hybrid / selective library reuse | Simple Sonarr and Radarr wrapper use cases | Python package | Lightweight Sonarr and Radarr Python wrapper | https://github.com/Kometa-Team/ArrAPI | https://arrapi.kometa.wiki/ | Selective simple library use where lighter syntax is preferable | Lighter alternative to PyArr |
| PlexAPI | Media automation library | Hybrid / reuse as library | Plex automation wrappers | Python package | Deep unofficial Plex Python control library | https://github.com/pkkid/python-plexapi | https://python-plexapi.readthedocs.io/ | Thin wrapper for custom Plex glue logic only | Primary low-level Plex Python library |
| Fetcharr | Watchlist sync candidate | Conditional / evaluate | Watchlist-sync lane | Yes if selected | Plex watchlist to Sonarr and Radarr sync product | https://github.com/Fetcharr/Fetcharr | Repository docs | Evaluate as one primary sync pattern candidate | Lightweight Docker-first sync option |
| Watchlistarr | Watchlist sync candidate | Conditional / evaluate | Watchlist-sync lane | Yes if selected | Real-time Plex watchlist sync to Sonarr and Radarr | https://github.com/nylonee/watchlistarr | Repository docs | Evaluate as one primary sync pattern candidate | Strong feature-rich watchlist-sync candidate |
| Pulsarr | Watchlist sync and routing candidate | Conditional / evaluate | Watchlist-sync lane | Yes if selected | Real-time Plex watchlist monitoring, routing, and notifications | https://github.com/jamcalli/Pulsarr | Public docs and built-in API docs | Evaluate as one primary sync pattern candidate | Strong multi-user and routing-heavy candidate |
| Plex Autoscan | Targeted scan optimization reference | Reference only / selective adopt | Media scan optimization | Yes if selected | Webhook-driven targeted Plex scan optimization | https://github.com/Cloudbox/autoscan | Repository docs | Use patterns or bounded deployment instead of full custom scan logic | Good optimization reference |
| syncarr | Multi-instance media sync reference | Reference only / selective adopt | Multi-instance routing and sync | Yes if selected | Sync multiple Radarr or Sonarr instances | https://github.com/syncarr/syncarr | Repository docs | Use if multi-instance sync becomes necessary | Strong 4K and 1080p sync reference |
