...
`config/service-registry.yaml` is read-only during the C5 transition
...
The `config/service-registry.yaml` entry is regenerated by the
...
# Block 4.C: NetBox is the CMDB; service-registry.yaml is regenerated.
