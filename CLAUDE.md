...
- **Backlog (deferred, not blocking):** (1) `roadmap-create.sh --update-existing` flag for closing IN PROGRESS rows when the gating artifact lands; (2) MCP-tool surface (`roadmap_create_with_artifact` exposed via xindex-mcp) once chat-attachment-pickup primitive is reliable in this surface — was Option (c) in the WP-02 surface decision, deferred on primitive-stability grounds. Option (b) (OpenProject attachment hook) explicitly rejected on doctrine inversion grounds (would invert D-16-02.A "repo-owned docs are canonical" by making OP the source of truth).
