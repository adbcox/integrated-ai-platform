#!/usr/bin/env bash
# Aider wrapper: execute task and capture schema-compliant artifacts
...
# Run Aider in the worktree
echo "[Aider] Starting task $TASK_ID in $WORKTREE"
cd "$WORKTREE"
...
if aider --no-auto-commits 2>&1 | tee "$RUN_DIR/execution.log"; then
  RESULT="success"
else
  EXIT_CODE=$?
  RESULT="failure"
...
echo "[Aider] Task $TASK_ID completed with status: $RESULT (exit $EXIT_CODE, ${DURATION}s)"
echo "[Aider] Artifacts written to: $RUN_DIR"
