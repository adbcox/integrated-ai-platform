...
def attestation_rollup_validator(input_dict):
    ...
    if "upstream_seal" in input_dict:
        if input_dict.get("upstream_seal") != "sealed":
            ...
            return {"attestation_rollup_status": "upstream_not_sealed"}
    if "validation_data" in input_dict:
        if not input_dict.get("validation_data"):
            # HARD GATE: Validation data missing or empty
            return {"attestation_rollup_status": "validation_failed"}
    return {"attestation_rollup_status": "ok"}
