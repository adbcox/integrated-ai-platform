"""DomainBranchFirstWaveRatifier — LAPC1 P10.

Ratifies Media Control, Media Lab, Meeting Intelligence to done or scaffold_complete_product_deferred.
Inspection gate confirmed:
  DomainBranchProofHarness public methods: evaluate, evaluate_batch
...
from framework.domain_branch_first_wave import (
    FIRST_WAVE_MANIFEST as _FIRST_WAVE_MANIFEST,
    FirstWaveDomainRunner as _FirstWaveDomainRunner,
)

assert hasattr(_DomainBranchProofHarness, "evaluate"), "INTERFACE MISMATCH: DomainBranchProofHarness.evaluate"
...
assert "criteria_passed" in _BranchProofResult.__dataclass_fields__, "INTERFACE MISMATCH: BranchProofResult.criteria_passed"
assert _FIRST_WAVE_MANIFEST.branch_count == 3, "INTERFACE MISMATCH: FIRST_WAVE_MANIFEST.branch_count"

_DEFAULT_ARTIFACT_DIR = Path("artifacts") / "first_wave_promotion"
...
@dataclass
class FirstWavePromotionRecord:
    branch_name: str
    verdict: str
    criteria_passed: int
    criteria_total: int
...
@dataclass
class FirstWavePromotionArtifact:
    records: list
    any_done: bool
...
def ratify_first_wave_promotion(
    *,
    artifact_dir: Optional[Path] = None,
    dry_run: bool = False,
) -> FirstWavePromotionArtifact:
    harness = _DomainBranchProofHarness()
    runner = _FirstWaveDomainRunner()
    results = harness.evaluate_batch(_FIRST_WAVE_MANIFEST, runner, repetitions=2)

    records = []
    for res in results:
        records.append(FirstWavePromotionRecord(
    ...
    artifact = FirstWavePromotionArtifact(
        records=records,
        any_done=any_done,
    ...
    if not dry_run:
        ...
        out_path = out_dir / "LAPC1_first_wave_promotion.json"
        out_path.write_text(
