...
def contract_stability_gate(input_dict):
    ...
    if "upstream_seal" in input_dict:
        if input_dict.get("upstream_seal") != "sealed":
            ...
            return {"contract_stability_gate_status": "upstream_not_sealed"}
    if "validation_context" in input_dict:
        if not input_dict.get("validation_context"):
            # HARD GATE: Validation context missing
            return {"contract_stability_gate_status": "validation_context_failed"}
    return {"contract_stability_gate_status": "ok"}
